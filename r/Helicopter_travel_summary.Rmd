...
# Calculate the average values within each workcentre/district footprint
...
ggplot(data = point_data %>% filter(!is.na(District)), aes(x = District, y = ttl_tm_s)) +
...
formattable(summary_district, align = c("l", "r"))

# Create box plots for workcentres
ggplot(data = point_data %>% filter(!is.na(Work_Centr)),
 ...
 ggtitle("Box Plot of helicopter return time by Workcentre")

# Create table for workcentres
formattable(summary_workcentre, align = c("l", "r"))
